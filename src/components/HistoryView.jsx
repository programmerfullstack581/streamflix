...
export default function HistoryView({
  ...
  return (
    <div className="space-y-6 animate-fadeIn w-full">
      ...
      <div className="p-6 sm:p-8 bg-white rounded-3xl border border-sky-100 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center space-x-3.5">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-sky-400 to-sky-600 flex items-center justify-center text-white shadow-md shadow-sky-400/20 flex-shrink-0">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Aquí puedes volver a descargar en MP3 o MP4 las canciones que ya has procesado.
            </p>
          </div>
        </div>
        ...
        {downloads.length > 0 && (
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-2xl text-xs font-bold transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-xs"
          >
      ...
      {downloads.length === 0 ? (
        <div className="p-10 sm:p-16 text-center bg-white rounded-3xl border border-sky-100 shadow-sm space-y-4 text-slate-500">
          <div className="w-16 h-16 rounded-3xl bg-sky-50 flex items-center justify-center mx-auto text-sky-400">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
          {filteredDownloads.map((track) => (
            <div
              key={track.videoId}
              className="p-4 bg-white hover:bg-sky-50/30 rounded-2xl border border-slate-200/80 hover:border-sky-200 transition-all flex items-center space-x-3.5 group shadow-xs hover:shadow-sm"
            >
              <img 
                ...
                className="w-14 h-14 rounded-xl object-cover flex-shrink-0 shadow-xs ring-1 ring-sky-100"
              />
              ...
              <div className="flex items-center space-x-1.5 flex-shrink-0">
                <button
                  ...
                  className="p-2 bg-gradient-to-r from-sky-400 to-sky-500 hover:from-sky-500 hover:to-sky-600 text-white rounded-xl shadow-xs hover:scale-105 transition-transform cursor-pointer"
